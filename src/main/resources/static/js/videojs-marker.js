  ...
  * Copyright (c) 2018 ; Licensed  */
    ...
    onMarkerClick: function onMarkerClick(marker) {
        ...
        xhttp.onreadystatechange = function() {
             if (this.readyState == 4 && this.status == 200) {
                 console.log("sent user click");
             }
        };
        xhttp.open("POST", "http://localhost:8089/sendClick", true); 
        xhttp.setRequestHeader("Content-type", "application/json");
        var myObj = new Object();
        myObj.contentId = marker.contentId;
        myObj.videoPartId = marker.videoPartId;
        myObj.userId = Cookies.get('userId');
  ...
  function registerVideoJsMarkersPlugin(options) {
    ...
    function setMarkderDivStyle(marker, markerDiv) {
      ...
      //markerDiv.style.backgroundImage= 'linear-gradient(to right, red , yellow)';
      if(firstColor == null){
    ...
    player.on("pause", function(event){
        ...
        console.log("pauseeeeeeeeeeee", event.clientX, event.clientY, player.currentTime());
    });
    var playTime = 0;
    var myInterval;
    player.on("play", function(event){
    	console.log("plaaaaaaaaaaaaaaay", event.clientX, event.clientY, player.currentTime());
    	myInterval = setInterval(myPlayTimer ,1000);
    	function myPlayTimer() {
    		playTime++;
    		console.log("Play Time is: " + playTime);
    	}
    });
